          ...
          <?php foreach ($posts as $post): ?>
            <article class="blog-card">
              <?php if ($post['featured_image']): ?>
                <img src="<?php echo htmlspecialchars($post['featured_image']); ?>" alt="<?php echo htmlspecialchars($post['title']); ?>" class="blog-img">
              <?php else: ?>
                <img src="https://images.unsplash.com/photo-1464983953574-0892a716854b?q=80&w=800&auto=format&fit=crop" alt="<?php echo htmlspecialchars($post['title']); ?>" class="blog-img">
              <?php endif; ?>
              <div class="blog-body">
                <h2 class="blog-title"><?php echo htmlspecialchars($post['title']); ?></h2>
                <p class="blog-meta">By <?php echo htmlspecialchars($post['author']); ?> | <?php echo date('M Y', strtotime($post['created_at'])); ?></p>
                <p class="blog-excerpt"><?php echo htmlspecialchars($post['excerpt'] ?: substr(strip_tags($post['content']), 0, 150) . '...'); ?></p>
                <a href="#" class="btn btn-small">Read More</a>
              </div>
            </article>
          <?php endforeach; ?>
        <?php else: ?>
          <!-- Default blog posts when no posts in database -->
          <article class="blog-card">
